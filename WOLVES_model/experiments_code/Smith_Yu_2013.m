...
parfor subject = 1:numSubjects
    ...
    visuals_On = 1;    visuals_Off = floor(4000/scale_factor); trial_Gap = floor(1000/scale_factor); %a gap of 1 sec between 2 trials
    word1_On  = floor(368/scale_factor);     word1_Off = floor((368+745)/scale_factor);
    word2_On  = floor(1850/scale_factor);    word2_Off = floor((1850+745)/scale_factor);
    ...
    Paired_combs_all = [Object_pairs_named ; Object_pairs_named_rev]; %all possible test tuples
    Sequence = 1:size(Object_pairs_named,1);
    ...
    for tr=1:maxTR
        %sim2.init();
        sim2.t =sim2.tZero;
        pos = ((randi(2)-1)*size(Object_pairs_named,1))+ Sequence(tr); % Finds postion of a random tuple
        ...
        while sim2.t <= t_max
            ...
            t = sim2.t;
            % missing STEP 1 Add sesame attention 3 second for first 4 trials
        ...
        %if ~mod(tr,5); tts('block ends','Microsoft Hazel Desktop - English (Great Britain)'); pause(5);end
...
%% SY tests
    visuals_On = 1; visuals_Off = floor(8000/scale_factor); trial_Gap = floor(1000/scale_factor); %a gap of 1 sec between 2 trials
...
%% 1 sec test   
%     visuals_On = 1;visuals_Off = floor(1000/scale_factor); trial_Gap = floor(1000/scale_factor); 
