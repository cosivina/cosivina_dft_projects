...
maxTR = 90; % total # of training trials
...
parfor subject = 1:numSubjects % change to parfor for mode = 2 
    ...
    word1_On  = floor(500/scale_factor);     word1_Off = floor(1500/scale_factor); % word 1 on off timing
    word2_On  = floor(2000/scale_factor);    word2_Off = floor(3000/scale_factor); % word 2 on off timing
    ...
    for j=1:nFeatures
        ...
        for i=1:nObjects
            ...
            %TempFe = [TempFe  Property{j}(3*i)]; %diagonal associations, use for an intuitive visualization 
        end
        Feature{j}=TempFe;
    end
    ...
    for i=1:nObjects
        ...
        %Words = [Words  Names(3*i)];    %diagonal associations, use for an intuitive visualization
    end
    ...
    Sequence = randperm(size(Object_pairs_named,1)); %generate a random sequence of 30 tuples
   
    finSequence {subject}= zeros(maxTR,1);
    for tr=1:size(Object_pairs_named,1) 
    ...
    for tr=1:maxTR %n trials loop
        sim2.t =sim2.tZero;
        pos = mod(tr,30);%mod(finSequence{subject}(tr)-1,size(Object_pairs_named,1))+1;
        ...
        while sim2.t <= t_max % run the ime loop of a trial
            ...
            if isalmost(t, word2_On,tolerance) %present audio stimuli 
                wrd2 = 2*nFeatures+2;
                sim2.setElementParameters({'Word_Label_2'}, {'position','amplitude'},... 
                    {(Paired_combs_all(pos,wrd2)),wstrength});
            end
            if isalmost(t, word2_Off,tolerance) %end audio stimuli 
                wrd2 = 2*nFeatures+2;
                sim2.setElementParameters({'Word_Label_2'}, {'position','amplitude'},... 
                    {0,0});
            end
