...
t_maxF = floor((2000+1000)/scale_factor);
t_max = floor((11250+1000)/scale_factor); %specify simulation time  % scale with Experiment training trial Duration   
...
parfor subject = 1:numSubjects
    ...
    visuals_On = 0;
    visuals_Off = floor(2000/scale_factor);
    words_On = floor(150/scale_factor); 
    ...
    for tr=1:maxFam
        %sim2.init();
        sim2.t =sim2.tZero;
        targ = tr; % setting first 3 words as known
        if tr > 1
            for i = 1 : nFeatures
                n=num2str(i);
        ...
        while sim2.t <= t_maxF
            ...
            if isalmost(t, t_maxF,tolerance)
                for i = 1 : nFeatures
                    n=num2str(i);
...
%               savestate_historyRt{subject}(tr,:) = sim2.getComponent('history lookRt', 'output');
 ...
 %% LEARNING PHASE   
    visuals_On = 0;
    visuals_Off = floor((11250)/scale_factor);
    word1_On  = floor(2250/scale_factor);% 
    word1_Off = floor(4450/scale_factor);%
    word2_On  = floor(4500/scale_factor);%
    word2_Off = floor(6700/scale_factor);%
    word3_On  = floor(6750/scale_factor);%
    word3_Off = floor(8950/scale_factor);%
    word4_On  = floor(9000/scale_factor);%
    word4_Off = floor(11250/scale_factor);%
   ...
   %% DESIGN TRAINING TRIALS
    ...
    training_order{subject}=zeros(1,8);% save locations of 4 objs on screen with 4 word sequence
    training_pair{subject}=zeros(size(Indices_Set));
    %% RUN TRAINING TRIALS
    for tr=1:maxTR
        sim2.t =sim2.tZero;
        ...
        obj=randperm(4);
        wrd=randperm(4);
        training_pair{subject}(tr,:)=Indices_Set(pos,:);
        training_order{subject}(tr,:)=[obj wrd];
        %disp(['train trial ', num2str(tr)]); disp(Indices_Set(pos,:)); disp([obj wrd]);
        for i = 1 : nFeatures
            n=num2str(i);
            handle_hcon_f{subject}(i).output = squeeze(savestate_hcon_f{subject}(i,:));
            handle_hwm_f{subject}(i).output = squeeze(savestate_hwm_f{subject}(i,:));
        ...
        while sim2.t <= t_max
            ...
            if isalmost(t, word3_On,tolerance)
                sim2.setElementParameters({'Word_Label_3'}, {'position','amplitude'},... 
                    {cell2mat(Words(Indices_Set(pos,wrd(3)))),wstrength});
            end
            if isalmost(t, word3_Off,tolerance) 
                sim2.setElementParameters({'Word_Label_3'}, {'position','amplitude'},... 
                    {cell2mat(Words(Indices_Set(pos,wrd(3)))),0});
            end
            if isalmost(t, word4_On,tolerance) 
                sim2.setElementParameters({'Word_Label_4'}, {'position','amplitude'},... 
                    {cell2mat(Words(Indices_Set(pos,wrd(4)))),wstrength});
            end
            if isalmost(t, word4_Off,tolerance) 
                sim2.setElementParameters({'Word_Label_4'}, {'position','amplitude'},... 
                    {cell2mat(Words(Indices_Set(pos,wrd(4)))),0});
            end
            if isalmost(t, visuals_Off,tolerance) 
                for f=1:nFeatures
                    n=num2str(f);                  
    ...
    matTrain.training_order = training_order{subject};
