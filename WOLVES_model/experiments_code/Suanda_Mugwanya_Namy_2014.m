...
parfor subject = 1:numSubjects
    ...
    word1_On  = floor(500/scale_factor);%250;% 125   
    word1_Off = floor(1500/scale_factor);%625;% 325
    word2_On  = floor(2000/scale_factor);%875;% 475
    word2_Off = floor(3000/scale_factor);%1250;%
    ...
    %% Condition MCD-Moderate
        Object_pairs=[];
        Object_pairs(1,:)= [1 2]; Object_pairs(2,:)= [2 1]; Object_pairs(3,:)= [1 3]; Object_pairs(4,:)= [1 4];
        Object_pairs(5,:)= [2 3]; Object_pairs(6,:)= [2 4]; Object_pairs(7,:)= [3 4]; Object_pairs(8,:)= [4 3];
        Object_pairs(9,:)= [5 6]; Object_pairs(10,:)= [6 5]; Object_pairs(11,:)= [5 7]; Object_pairs(12,:)= [5 8];
        Object_pairs(13,:)= [6 7]; Object_pairs(14,:)= [6 8]; Object_pairs(15,:)= [7 8]; Object_pairs(16,:)= [8 7];
    elseif mod(subject,3)==0
    %% Condition LCD-Low
        Object_pairs=[];
        Object_pairs(1,:)= [1 2]; Object_pairs(2,:)= [2 1]; Object_pairs(3,:)= [1 2]; Object_pairs(4,:)= [1 3];
        Object_pairs(5,:)= [2 4]; Object_pairs(6,:)= [3 4]; Object_pairs(7,:)= [3 4]; Object_pairs(8,:)= [4 3];
        Object_pairs(9,:)= [5 6]; Object_pairs(10,:)= [6 5]; Object_pairs(11,:)= [5 6]; Object_pairs(12,:)= [5 7];
        Object_pairs(13,:)= [6 8]; Object_pairs(14,:)= [7 8]; Object_pairs(15,:)= [7 8]; Object_pairs(16,:)= [8 7];
    end
        ...
        while sim2.t <= t_max
            ...
            if isalmost (t, word1_On, tolerance)
                sim2.setElementParameters({'Word_Label_1'}, {'position','amplitude'},... 
                    {cell2mat(Words(Object_pairs(pos,inW(1)))),wstrength});
            end
            if isalmost (t, word1_Off, tolerance)
                sim2.setElementParameters({'Word_Label_1'}, {'position','amplitude'},... 
                    {cell2mat(Words(Object_pairs(pos,inW(1)))),0});
            end
            if isalmost (t, word2_On, tolerance)
                sim2.setElementParameters({'Word_Label_2'}, {'position','amplitude'},... 
                    {cell2mat(Words(Object_pairs(pos,inW(2)))),wstrength});
            end
            if isalmost (t, word2_Off, tolerance)
                sim2.setElementParameters({'Word_Label_2'}, {'position','amplitude'},... 
                    {cell2mat(Words(Object_pairs(pos,inW(2)))),0});
            end
