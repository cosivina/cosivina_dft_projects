...
t_max = floor((9000/2+1000)/scale_factor); %specify simulation time  % scale with Experiment training trial Duration   
...
parfor subject = 1:numSubjects
    ...
    visuals_On = 0;
    visuals_Off=floor(9000/(2*scale_factor));
    word1_On  = floor(1000/(2*scale_factor));% values assumed, i could not find in the article  
    word1_Off = floor(2000/(2*scale_factor));%
    word2_On  = floor(4000/(2*scale_factor));%
    word2_Off = floor(5000/(2*scale_factor));%
    word3_On  = floor(7000/(2*scale_factor));%
    word3_Off = floor(8000/(2*scale_factor));%
    ...
    Object_indB=1+mod(1:nObjects,nObjects);  % B C D E...
    Object_indD=1+mod(3:nObjects+2,nObjects);% D E F G...
    Object_indE=1+mod(4:nObjects+3,nObjects);% E F G H...
    Object_indJ=1+mod(9:nObjects+8,nObjects);% J K L M...
    
    Indices_Set=[Object_indA' Object_indB' Object_indD'; Object_indA' Object_indE' Object_indJ'];
            ...
            if t == visuals_On                
                for f=1:nFeatures
                    ...
                    sim2.setElementParameters({ strcat('Feature_',num2str(f),'_Left'), ... % set feature dimension value
                        ...
                        {cell2mat(Feature{f}(Indices_Set(pos,obj(1)))),...
                        cell2mat(Feature{f}(Indices_Set(pos,obj(2)))),cell2mat(Feature{f}(Indices_Set(pos,obj(3))))});                
                    ...
                    sim2.setElementParameters({ strcat('Feature_',num2str(f),'_Left'), ... % set spatial dimension value
                        strcat('Feature_',num2str(f),'_Centre'), strcat('Feature_',num2str(f),'_Right')}, ...
                         {'positionX', 'positionX', 'positionX'}, {spt_L,spt_C,spt_R});
