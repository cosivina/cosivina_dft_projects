...
simName = ['hwf_wf_' num2str(parVal) '_xsit_sub_']; %''
...
for subject=1:numSubjects
    figure(19);  
    lsp= subplot(1,2,1);
    surface(lsp,squeeze(xsit_result.train(subject).hwf(1,:,:)));
    title(lsp,'Train hwf');
    caxis([0, 1])
    colorbar
    shading flat
    rsp= subplot(1,2,2);
    surface(rsp,squeeze(xsit_result.test(subject).hwft(1,:,:)));
    title(rsp,'Test hwf');
    caxis([0, 1])
    colorbar
    shading flat
    if goodLearners(subject)==1, suptitle('Strong'), elseif goodLearners(subject)==0, suptitle('Weak'),end
