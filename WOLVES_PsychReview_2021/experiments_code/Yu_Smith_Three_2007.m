...
t_max = floor((9000+1000)/scale_factor); %specify simulation time  % scale with Experiment training trial Duration   
...
parfor subject = 1:numSubjects
    ...
    visuals_On = 0;
    visuals_Off=floor(9000/(scale_factor));
    word1_On  = floor(1000/(scale_factor));% values assumed, i could not find in the article  
    word1_Off = floor(2000/(scale_factor));%
    word2_On  = floor(4000/(scale_factor));%
    word2_Off = floor(5000/(scale_factor));%
    word3_On  = floor(7000/(scale_factor));%
    word3_Off = floor(8000/(scale_factor));%
